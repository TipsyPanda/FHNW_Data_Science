...
library(here)
require(xgboost)
#Increase JVM memory size to 16 to support weka.
options(java.parameters = "-Xmx16g")
getwd()
lcdata <- readRDS("../Data/Out/Cleandata.rds")
#lcdata <- data.frame(lcdata %>% dplyr::select(int_rate, term, loan_amnt,dti_merged,emp_length,term ,installment,home_ownership,purpose, revol_util,revol_bal))
lcdataC <- data.frame(lcdata %>% dplyr::select(where(is.character)))
lcdata <- lcdata %>% dplyr::select(-emp_title,-emp_length,-home_ownership,-issue_d,-purpose,-title,-zip_code,-addr_state,-earliest_cr_line)
...
As a first step the class attribute "int_rate" is analyzed:

```{r}
#Check Boxplot and histogram
boxplot(lcdata$int_rate, main ="Boxplot with outliers")
hist(lcdata$int_rate)
summary(lcdata$int_rate)
```
...
set.seed(713)
trainIndex <- initial_split(lcdata, prop = 0.5) #stratify the class attribute since it is right skewed.This bins the data into quartiles
trainingSet <- training(trainIndex)
testSet <- testing(trainIndex)
```

### further pre-process the data to be used with our models
...
xgb_grid <- grid_latin_hypercube(
  ...
  finalize(mtry(), trainingSet_processed), #treat mtry() differently because it depends on the actual number of predictors in the data.
  learn_rate(),
  size = 30 #can be set to a higher value to improve results but is very time consuming
)
...
Below we see a visualization of the tuning grid runs. On the Y-Axis the RMSE is shown and the X-Axis shows the tuned hyperparamter values. - Increased learn rate reduces the RMSE - in the rest of the attributes no clear correlation is visible. There are a couple of noticable data points such the min_n at 15 and 35. With increased size of the tuning grid this could get more visible.
...
In a final step we analyze our model and try to intrepret it.
...
```{r plot train vs test, echo=FALSE}
all_predictions %>%
...
We know that we have fewer calls during the night. This explains the graph below where we can see that during the night, when we have a low number of calls, the absolute error is higher.

```{r plot absolut error per hour}
all_predictions %>%
  dplyr::filter(type=="test")%>% 
  ggplot(aes(Hour, AE)) +
  geom_point() 
...
```{r summary of predictions}
all_predictions %>%
  filter( as.integer(Hour) >=7 & as.integer(Hour) <=19 & type =="test")%>%
  group_by(type) %>%
...
The model can now be used to make predictions. In the example below it can be seen that on the 3rd of December at 17:00 you can expect ca. 30 calls more per hour depending or not whether it is a holiday.
...
We have found a model that can predict the number of incoming calls per hour in a reasonable level of accuracy for the given use case. To further improve the model the following options are available:
...
The following tutorials where used as a reference for this report: Tidymodels Tutorial by John Ross: <https://rpubs.com/jwross83/933687> Tidymodels: tidy machine learning in R by Rebecca Barter: <https://www.rebeccabarter.com/blog/2020-03-25_machine_learning/>
