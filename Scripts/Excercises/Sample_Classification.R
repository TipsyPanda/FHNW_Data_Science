...
# Convert training and test data to k-NN specific fomrat
...
glm.fit <- glm(default~income + student,family="binomial", data=training.data)
summary(glm.fit)
...
new.data <- data.frame(student = c("No", "No"), balance= c(250, 2500), income=c(1000, 2000)) # student and income are arbitrarily set, since they will not be used by predict
...
# Predicting probabilities and classes for a balance of 1000 and 2000 Dollars:
new.data <- data.frame(student = c("No", "Yes"), balance= c(250, 2500), income=c(1000, 2000)) # student and income are arbitrarily set, since they will not be used by predict
predict(glm.fit, newdata = new.data, type = "response")
