...
matrix_data <- subset(data, select = -c(status))
matrix_targets <- data.matrix(data$status)
...
k <- 10
indices <- sample(1:nrow(matrix_data))
folds <- cut(indices, breaks = k, labels = FALSE)
...
for (i in 1:k) {
  ...
  val_indices <- which(folds == i, arr.ind = TRUE)
  val_data <- matrix_data[val_indices,]
  val_targets <- matrix_targets[val_indices]

  partial_train_data <- matrix_data[-val_indices,]
  partial_train_targets <- matrix_targets[-val_indices]
