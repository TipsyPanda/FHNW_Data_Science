...
# Try to create data matrix
#dataset1 <- data
#c(c(train_data, dataset1$status), c(test_data, test_targets)) %<-% dataset1
...
test_data <- scale(data, center = mean, scale = std)
...
for (i in 1:k) {
  ...
  val_indices <- which(folds == i, arr.ind = TRUE)
  val_data <- subset(data[val_indices,], select = -c(status))
  val_targets <- data$status[val_indices]

  partial_train_data <- subset(data[-val_indices,], select = -c(status))
  partial_train_targets <- data$status[-val_indices]
