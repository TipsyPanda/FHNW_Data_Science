...
set.seed(12)
preprocRecipe <-
  recipe(status ~., data = trainingSet) %>%
...
matrix_data <- trainingSet_processed %>% select(-tail(names(trainingSet_processed), 7))
#matrix_data <- subset(data, select = c(CNT_CHILDREN, AMT_INCOME_TOTAL))
#matrix_targets <- data.matrix(trainingSet_processed[])
matrix_targets <- trainingSet_processed %>% select(tail(names(trainingSet_processed), 7))
...
test_data <- data.matrix(matrix_data)
...
#write.csv(average_mae_history, "../Doc/Versuch 1/Try 1.csv", row.names=FALSE)
